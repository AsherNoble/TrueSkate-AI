...
def test_checkpoint_evaluation_honours_the_trained_dataset_shape():
    ...
    for block in blocks:
        ...
        while (found := block.find("BasicLinearClipDataset(", cursor)) != -1:
            ...
            cursor = found + 1
    # 14 = 13 checkpoint-backed evaluators plus the one orange-cue exception above.
    # Bumping this deliberately is the point: a new evaluator cannot land without
    # being seen here.
    assert constructions == 14, f"expected 14 dataset constructions, found {constructions}"
