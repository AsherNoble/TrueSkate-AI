...
The inference core is `trueskate_ai.model2.infer` (pure torch). This script is the
thin device wrapper.

Usage:
    # dry run — full inference + param-vector decode on synthetic frames, NO device
    python scripts/model2/run_sequence_policy.py --dry-run --steps 5
    python scripts/model2/run_sequence_policy.py --dry-run --model notebooks/models/sequence_model.pth

    # on device — needs WDA running (python scripts/launch_services.py)
    python scripts/model2/run_sequence_policy.py --model notebooks/models/sequence_model.pth --steps 20
"""
...
from trueskate_ai.model2.infer import SequencePolicyRunner, TimestampedMjpegBuffer, load_policy  # noqa: E402
from trueskate_ai.model2.policy import SequencePolicy, SequencePolicyConfig  # noqa: E402
...
def _on_device(args) -> None:
    ...
    worker = DeviceSession(dev, calibrate_touch_on_connect=False)
    worker.connect()
    runner = SequencePolicyRunner(model, cfg, device)
