...
Run (repo root, ~/.modal.toml auth):
    .venv/bin/modal run scripts/model1/train_trace_extractor_modal.py --smoke
        # cents: CPU container, synthetic one-step train + volume/import checks
    .venv/bin/modal run scripts/model1/train_trace_extractor_modal.py --data-subdir <session>
        # PAID A10G mini-run on one session
    .venv/bin/modal run scripts/model1/train_trace_extractor_modal.py --latency-s 0.2
...
image = (
    ...
    .add_local_file(
        str(_REPO_ROOT / "scripts" / "model1" / "train_temporal_trace_extractor.py"),
        remote_path="/root/scripts/model1/train_temporal_trace_extractor.py",
    )
    .add_local_file(str(_REPO_ROOT / "scripts" / "model1" / "train_trace_extractor.py"),
                    remote_path="/root/scripts/train/train_trace_extractor_legacy.py")
    .add_local_file(str(_REPO_ROOT / "scripts" / "model2" / "build_bc_clips.py"),
                    remote_path="/root/scripts/model2/build_bc_clips.py")
)
...
def _load_trainer():
    ...
    spec = importlib.util.spec_from_file_location(
        ...
        "/root/scripts/model1/train_temporal_trace_extractor.py",
    )
...
def _load_build_bc_clips():
    ...
    import sys
    spec = importlib.util.spec_from_file_location(
        "build_bc_clips", "/root/scripts/model2/build_bc_clips.py")
    mod = importlib.util.module_from_spec(spec)
...
def audit_selection_remote(data_match: str = "supercrown", max_samples: int = 100) -> dict:
    ...
    from trueskate_ai.model1.temporal.dataset import discover_sample_paths
