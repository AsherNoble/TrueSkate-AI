...
.video-shell {{ width: min(100%, 520px); height: min(72vh, 760px); display: grid; place-items: center; background: #050607; border: 1px solid #30363c; border-radius: 8px; overflow: hidden; }}
video {{ width: 100%; height: 100%; object-fit: contain; image-rendering: auto; }}
...
function load() {{
  ...
  el('jump').value=cursor+1; updateFrame();
}}
...
video.addEventListener('loadedmetadata',updateFrame); video.addEventListener('timeupdate',updateFrame); video.addEventListener('seeked',updateFrame);
...
visible=DATA.samples.slice(); load();
</script>
</body></html>"""
