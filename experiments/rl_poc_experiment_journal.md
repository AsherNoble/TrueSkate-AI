...
- **Verified offline:** action_param spin round-trip, optimizer setup (len-29 bounds/sigma/mean for big_spin), warm-start spin-block append, mixed legacy+spin mining round-trip. **GATE NOT YET PASSED:** on-device OCR confirmation that shove+spin → a SPIN_FAMILY landed trick must run before any unattended big_spin run; retarget if the emergent trick differs.
